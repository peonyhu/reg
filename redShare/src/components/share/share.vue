...
//  import {weiShare} from '@/common/js/wx_share.js';
  export default{
    data(){
      return {
        share: {
    ...
    methods:{
      ...
      shareInfo(){
          ...
          this.$http.get('/redPacket/api.php?action=shareInfo'+'&gid='+16,{
            ...
            let endTime = Math.floor(new Date(this.share.endTime).getTime()/1000);
            if(this.share.env != ''){
              if(oTime > endTime){
                if(this.share.env == 'productiona'){
                  window.open('/thanksgiving/missPage.php?gid="+16+"&title="+encodeURIComponent(this.share.title)+"&share="+encodeURIComponent(this.share.link)');
//                  window.location.href="/thanksgiving/missPage.php?gid="+16+"&title="+encodeURIComponent(this.share.title)+"&share="+encodeURIComponent(this.share.link);
